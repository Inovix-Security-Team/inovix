...
class EventRepository:
    ...
    def create_event(self, event: EventRecord) -> Optional[EventRecord]:
        query = """
            INSERT INTO events (id, timestamp, event_type, source, content_hash, metadata)
            VALUES (?, ?, ?, ?, ?, ?)
        """
        metadata_str = json.dumps(event.metadata) if isinstance(event.metadata, dict) else event.metadata
        with self.conn:
            self.conn.execute(query, (
                event.id, event.timestamp, event.event_type,
                event.source, event.content_hash, metadata_str
            ))
    ...
    def get_event(self, event_id: str) -> Optional[EventRecord]:
        ...
        return EventRecord(
            id=row["id"], timestamp=row["timestamp"], event_type=row["event_type"],
            source=row["source"], content_hash=row["content_hash"],
            metadata=parsed_meta, created_at=row["created_at"]
        )
